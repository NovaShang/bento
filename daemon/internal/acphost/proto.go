...
// session ⊃ window ⊃ pane tree lives with the daemon, so an app restart or
// another paired device reads the same
...
// workbench's terminal pane. `spawn` with
...
// on the ordinary attach/detach/credit ops under the raw-byte pane wire
...
// only lifecycle op (contrast an ACP agent, whose lifecycle belongs to
// `structure`).
//
//	{"op":"resize","agent_id":"pty:<uuid>","cols":C,"rows":R}
//	    pty resize ioctl; the kernel delivers SIGWINCH and a full-screen
//	    TUI repaints for the new geometry. Acked with the shapes the
...
type Control struct {
	...
	// with every existing client); "pty" starts a daemon-hosted pty process
	// (the pty extension above). Unknown kinds are refused, never
	// defaulted.
