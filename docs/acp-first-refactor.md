...
- **S4c 完成（5b06921 / c2b6765）**：menubar 真-tmux 栈整删（TmuxCLI/TmuxResolver/TmuxSession/TerminalAppKind；attach→focusOrOpen+store，wizard 只剩内建 ACP 路径，新增中立 SessionItem 菜单模型，preset 改用 core 单一来源）；mac 终端入口全部摘除（Plain Shell/SSH 子菜单/⌘⇧T no-tmux/BENTO_OPEN_WINDOW=plain；`newCommandWindow` 保留给 onboarding 安装流）；文案 "New tmux Window"→"New Pane"；BentoMenubar 代码 tmux 符号清零（仅 1 处 doc 注释）。Legacy/ 首个入驻 SSHConfigHosts_macOS；其余候选（SSHService/SSHKeyGenerator/AgentStatusRules/StateDetection/PredictiveEcho/Stripper/TurnNavigator/ScrollReviewCompose）仍有活跃类型级引用，留原地（清单在 S4c 报告）。并行冲突处理范例：menubar SettingsView 用 hash-object/update-index 只提交自身删改，UX session 的新增完好保留。
- **S4c 推迟项完成（665fabb / 14b387a）**：Tmux*/Window* 时代命名全部中立化（SessionStartChoice/.rawShell、activeSessionName、PaneSidebar/PaneTabBar/PaneDisplayStatus/PaneSeed、newFocusPane、SessionLister、AgentLayout、selectPane* selector 等 42 文件；UserDefaults key 与遥测事件名冻结；TipCenter rawValue 不变）；用户可见字符串去 tmux/window 措辞；死代码 AgentSpec.setupScript+测试删除；iOS 收口：Add SSH host 入口与 Welcome 的 SSH path 卡移除（既有 host 行保留，connect 仍报不支持）。验收 grep：活跃代码 tmux 残留=28 行历史注释。
- **S5 进度**：持久化 e2e 全绿（daemon/CLI HEAD 重编译重启后 acp-host-probe ALL OK）；mac 测试实例跑 HEAD 全量构建；余：mac 目检+手机安装+逐屏验收（用户侧）。
