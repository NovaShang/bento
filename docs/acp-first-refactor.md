...
- **S4c 推迟项（安静窗口做）**：`TmuxStartChoice`/`activeTmuxSessionName`/`refreshTmuxSessions`/`newTmuxWindow` selector/`TmuxLayout`（现有 AgentLayout typealias）等视图可见残名统一改；iOS HostListView direct-SSH 入口收口。
