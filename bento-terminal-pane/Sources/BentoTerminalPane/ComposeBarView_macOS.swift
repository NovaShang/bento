...
final class ComposeBarView: NSView {
    ...
    // Appearance-aware ink/background: the draft bar sits over the terminal, so
    // it tracks the app's light/dark like the rest of the pane chrome. Pushed by
    // the surface from its `TerminalTheme.isDark` (the term-era bar read the
    // theme store directly; the rendering base takes it by injection).
    var isDark: Bool = true { didSet { if isDark != oldValue { needsDisplay = true } } }
