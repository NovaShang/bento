...
/// Master switch for path detection under the pointer. Private copy of the
/// term-era `PathPreviewSettings` (bento-terminal-core FilePreviewCore.swift)
/// under the SAME defaults key, so the toggle the settings UI writes keeps
/// governing the rendering base; default on.
