...
// BentoTerminalPane — the terminal RENDERING base.
//
// Where this package sits in the module map (docs/architecture.md,
// docs/tmux-host-design.md step 6): it is the piece of the frozen terminal
// product that BOTH products' terminal-shaped panes need — the libghostty
// surface views (GhosttyTerminalSurface, iOS + macOS), the runtime that hosts
// them (GhosttyRuntime), and the pure logic the surfaces lean on: path/link
// detection, the output-parse state fallback (for panes with no protocol
// truth), predictive echo, scroll-review-compose, mouse-report encoding, and
// the tmux sizing vocabulary. bento-terminal-core keeps its own copy of every
// file here until the frozen product retires — the same deliberate duality as
// desktop-term/ beside desktop/.
//
// The contract is pane-shaped and constructor-injected: bytes in (`feed`),
// bytes out (`onInput`), resize (`onSizeChanged`), and title/pwd/link
// callbacks. The package depends on GhosttyKit alone — it knows nothing of
// workspaces, stores, ACP, theming stores, or file previews; hosts hand in a
// `TerminalTheme` and wire the callbacks. Anything that would have needed a
// trunk module (preview opening, path resolution, appearance) is a closure
// the host injects instead.
