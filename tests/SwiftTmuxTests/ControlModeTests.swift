...
struct NotificationTests {
    ...
    /// Modern form — what tmux 3.x actually emits (`%%session-renamed $%u %s`).
    /// Regression: this used to `dropFirst(18)` against a 17-character prefix,
    /// so the name came back with the session id still glued to it.
    @Test func sessionRenamedModernForm() {
        let (s, c) = makeService()
        s.feedData(Data("%session-renamed $3 my-session\n".utf8))
        if case .sessionRenamed(let session, let name) = c.last {
            #expect(session == TmuxSessionID(string: "$3"))
            #expect(name == "my-session")
        } else { Issue.record("Expected sessionRenamed") }
    }

    /// Names may contain spaces — the id splits off with maxSplits, and
    /// everything after the first space is the name.
    @Test func sessionRenamedNameWithSpaces() {
        let (s, c) = makeService()
        s.feedData(Data("%session-renamed $0 my session name\n".utf8))
        if case .sessionRenamed(_, let name) = c.last {
            #expect(name == "my session name")
        } else { Issue.record("Expected sessionRenamed") }
    }

    /// Legacy bare-name form: no id, and the first character survives.
    @Test func sessionRenamedLegacyForm() {
        let (s, c) = makeService()
        s.feedData(Data("%session-renamed work\n".utf8))
        if case .sessionRenamed(let session, let name) = c.last {
            #expect(session == nil)
            #expect(name == "work")
        } else { Issue.record("Expected sessionRenamed") }
