...
// Renamed from "GhosttyKit": SPM demands target names unique across the
// whole graph, and the trunk package owns the canonical name now. The
// xcframework's MODULE is still GhosttyKit, so imports are untouched.
let ghosttyKit: Target = .binaryTarget(
    name: "GhosttyKitFrozen",
