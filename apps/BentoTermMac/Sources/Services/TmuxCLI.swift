...
enum TmuxCLI {
    ...
    /// Test-only socket override, mirroring the daemon's BENTO_TMUX_SOCKET:
    /// production (unset) speaks to the DEFAULT tmux server — the same one
    /// the daemon's tmux host and the user's own terminal use; a test run
    /// pins a throwaway -L socket so it can never touch the real server.
    private static var socketFlags: [String] {
        guard let sock = ProcessInfo.processInfo.environment["BENTO_TMUX_SOCKET"],
              !sock.isEmpty else { return [] }
        return ["-L", sock]
    }
    ...
    static func kill(session: String) async throws {
        ...
        _ = try await runCapture(tmux, socketFlags + ["kill-session", "-t", "=" + session])
    }

    /// Rename a tmux session (exact-name target, same as `kill`).
    static func rename(session: String, to newName: String) async throws {
        guard let tmux = locate() else { return }
        _ = try await runCapture(tmux, socketFlags + ["rename-session", "-t", "=" + session, newName])
    }
    ...
    private static func captureRows(_ args: [String]) async -> [[String]]? {
        ...
        guard let result = try? await runCapture(tmux, socketFlags + args), result.code == 0 else {
            return nil
        }
