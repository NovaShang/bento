...
public final class AgentWorkspaceStore {
    ...
    package struct PaneEntry: Codable {
        package var id: Int
        package var kind: PaneKind = .acp
        package var presetID: String
        package var customPreset: ACPAgentPreset?
        ...
        package init(id: Int, kind: PaneKind = .acp, presetID: String,
    ...
    package struct WorkspaceEntry: Codable {
        package var id: Int
        package var name: String
        package var panes: [PaneEntry]
        package var layout: LayoutTree.Node
        package var activePane: Int
        /// Temporarily maximized pane (zoom); nil = normal tiling.
        package var zoomedPane: Int?
        package var cols: Int
        package var rows: Int
        /// Last mutation / agent activity — the menubar session list's
        /// relative-time column.
        package var lastActivity: Date = Date()

        private enum CodingKeys: String, CodingKey {
            case id, name, panes, layout, activePane, zoomedPane, cols, rows, lastActivity
        }

        package init(id: Int, name: String, panes: [PaneEntry], layout: LayoutTree.Node,
    ...
    package struct State: Codable {
        ...
        package var schema: Int = 2
        package var sessions: [WorkspaceEntry] = []
        package var nextPane = 1
        package var nextSession = 1
    ...
    package static func decodeState(_ data: Data) -> (state: State, migrated: Bool)? {
    ...
    package static let defaultCols = 160
    package static let defaultRows = 48
    ...
    package let mirror = WorkspaceMirrorState()
    ...
    package var catalog = SessionCatalog()
    ...
    package static func catalogKey(forWorkspaceKey key: String) -> String {
    ...
    package func workspace(_ name: String) -> WorkspaceEntry? {
        state.sessions.first { $0.name == name }
    }

    /// Session containing a pane (pane ids are globally unique).
    package func workspaceName(ofPane paneID: Int) -> String? {
    ...
    package func paneEntry(_ paneID: Int) -> PaneEntry? {
    ...
    /// The agent a bare seed (no command) runs — forwarded to the catalog
    /// (`AgentDefaults`), kept here so the app-facing API reads the same.
    public static var defaultPreset: ACPAgentPreset { AgentDefaults.defaultPreset }
    ...
    static func apiKeyPreset(matching commandOrID: String) -> ACPAgentPreset? {
        AgentDefaults.apiKeyPreset(matching: commandOrID)
    }
    ...
    public static func setDefaultAgentID(_ id: String) {
        AgentDefaults.setDefaultAgentID(id)
    }
    ...
    static var commandAliases: [String: String] { AgentDefaults.commandAliases }
    ...
    @discardableResult
    package func spawn(paneID: Int) -> (any PaneRuntime)? {
    ...
    package func reconnectPane(_ paneID: Int) {
    ...
    package func noteSpawned(paneID: Int, instanceID: String?, acpSessionID: String?) {
        guard let name = workspaceName(ofPane: paneID) else { return }
        withWorkspace(name) { sess in
            guard let p = sess.panes.firstIndex(where: { $0.id == paneID }) else { return }
