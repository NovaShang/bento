...
package final class WorkspaceMirrorState {
    ...
    /// "pushed" means.
    /// Dirty = the projected CONTENT differs. The comparison is byte-based,
    /// so the encoder must be byte-deterministic: Foundation's JSON key
    /// order is not guaranteed stable between encodes (it flipped when the
    /// package was re-rooted), and an unlucky order would read as a
    /// spurious unpushed edit. sortedKeys takes luck out of it.
