...
extension Notification.Name {
    // Raw strings match what the app-side theme store posts (trunk BentoUI's
    // ThemeStore uses the same names the term-era store did), so the runtime
    // recolors live without this package importing any store. Internal on
    // purpose: a host importing both modules must not see two competing
    // `.terminalThemeChanged` shorthands.
...
/// What the runtime needs to know about the app's terminal appearance. The
/// term-era runtime read `ThemeStore.shared` directly; the rendering base
/// takes the same facts by injection so it depends on no theme store.
public struct GhosttyRuntimeAppearance: Equatable, Sendable {
    /// Explicit colors, or nil to leave ghostty's built-in defaults in place
    /// (the term-era system-adaptive "System" theme writes no palette).
    ...
    /// nil → ghostty's default font.
    public var fontFamily: String?
    /// ≤ 0 → ghostty's default size. (Font SIZE is per-surface at creation;
    /// this app-wide copy only keeps a live config reload from resetting open
    /// surfaces — see `reapplyColors`.)
    public var fontSize: Double
    /// Whether the app-wide appearance reads as dark (drives the engine's
    /// color-scheme report to programs inside the terminal).
    public var isDark: Bool

    public init(palette: Palette? = nil, fontFamily: String? = nil,
                fontSize: Double = 0, isDark: Bool = true) {
...
public final class GhosttyRuntime {
    ...
    /// Host-injected appearance. Set BEFORE the first surface is created (the
    /// runtime is a lazy singleton — the first surface's init writes the color
    /// config), then post `.terminalThemeChanged` / `.terminalFontChanged` to
    /// re-apply live. The default renders ghostty's own dark look.
    public static var appearanceProvider: () -> GhosttyRuntimeAppearance = { .init() }
    ...
    private init() {
        ...
        GhosttyRuntime.writeColorConfig(GhosttyRuntime.appearanceProvider())
        #endif
    ...
    /// A nil palette writes no colors → ghostty uses its built-in defaults.
    private static func writeColorConfig(_ appearance: GhosttyRuntimeAppearance) {
        ...
        if let palette = appearance.palette {
            lines.append(String(format: "background = %06X", palette.background))
            lines.append(String(format: "foreground = %06X", palette.foreground))
            lines.append(String(format: "cursor-color = %06X", palette.cursor))
            for (i, c) in palette.ansi.prefix(16).enumerated() {
    ...
    private func reapplyColors() {
        ...
        let appearance = GhosttyRuntime.appearanceProvider()
        GhosttyRuntime.writeColorConfig(appearance)
        ...
        ghostty_app_set_color_scheme(
            app,
            appearance.isDark ? GHOSTTY_COLOR_SCHEME_DARK : GHOSTTY_COLOR_SCHEME_LIGHT)
