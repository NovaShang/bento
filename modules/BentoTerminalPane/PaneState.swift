...
private let profileLog = Logger(subsystem: "com.bento.terminalpane", category: "profiles")

/// The three states output-parse detection can conclude for a terminal pane.
///
/// This is the DETECTION vocabulary, not the display language: the trunk's
/// `BentoUI.PaneState` owns the palette and the chrome semantics, and its
/// `awaitingInput` carries no payload. This one keeps the term-era
/// `profile:` payload because detection needs to say WHICH profile matched
/// (that id selects the quick keys). The term-era palette extension stayed
/// behind — a host maps these cases onto `BentoUI.PaneState` for color.
