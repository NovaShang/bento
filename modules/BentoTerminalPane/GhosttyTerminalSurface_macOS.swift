...
public final class GhosttyTerminalSurface: NSView, TerminalSurface, NSTextInputClient {
    ...
    // menu). The term-era surface held a `PathPreviewContext` and opened the
    // preview itself (`BentoTerminalWindow.openPreview` / `SmartPathResolver`);
    // the rendering base only DETECTS — opening and filesystem resolution are
    // the host's, injected below. `onOpenPath == nil` = the affordance is off,
    // exactly like the old nil context.
    /// Open a confirmed path hit (host shows its preview UI). nil = off.
    public var onOpenPath: ((_ path: String, _ line: Int?) -> Void)?
    /// Resolve ambiguous ⌘click candidates against the pane's filesystem
    /// (the term-era `SmartPathResolver.resolveFirst`): ordered paths +
    /// root hints in, the first candidate that exists out (nil = none).
    /// When unset, only self-contained explicit tokens (`fastPath`) open.
    public var resolvePathClick:
        ((_ paths: [String], _ rootHints: [String]) async -> (index: Int, resolvedPath: String)?)?
    ...
    public func applyTheme(_ theme: TerminalTheme) {
        ...
        syncColorScheme()
        composeBar?.isDark = theme.isDark
        composeBar?.monoFont = composeFont()
    ...
    func pathHit(at point: NSPoint) -> SurfacePathHitEngine.Hit? {
        guard onOpenPath != nil, !isTornDown,
    ...
    /// immediately (the host's panel shows not-found if it lied); everything
    /// else — bare relatives, wrap-chain joins, truncated suffixes — resolves
    /// through the host's `resolvePathClick`, and the first candidate that
    /// exists wins.
    private func handlePathClick(_ scan: SurfacePathHitEngine.TapScan) {
        guard let open = onOpenPath else { return }
        clearPathHover()
        let hits = scan.hits
        if hits[0].fastPath {
            open(hits[0].path, hits[0].line)
            return
        }
        guard let resolve = resolvePathClick else { return }
        pathClickSeq += 1
        let seq = pathClickSeq
        Task { @MainActor [weak self] in
            guard let res = await resolve(hits.map(\.path), scan.rootHints) else { return }
            guard let self, self.pathClickSeq == seq, !self.isTornDown else { return }
            self.onOpenPath?(res.resolvedPath, hits[res.index].line)
        }
    }
    ...
    private func updatePathHover(_ event: NSEvent) {
        guard onOpenPath != nil, let cell = cellSizePoints() else { return }
        let p = convert(event.locationInWindow, from: nil)
    ...
    /// Open the preview (host UI) for a confirmed hit.
    private func presentPathPreview(_ hit: SurfacePathHitEngine.Hit) {
        guard let open = onOpenPath else { return }
        clearPathHover()
        open(hit.candidate.path, hit.candidate.line)
    }
...
/// Hover highlight for a detected path token — rounded wash + underline over
/// the rows the token crosses. Private copy of the term-era view (it lived in
/// bento-terminal-core's FilePreviewPanel_macOS.swift); it draws over the
/// terminal, so it belongs to the rendering base, not the preview stack.
