...
public final class TmuxControlMode: @unchecked Sendable {
    ...
    /// Modern form `$id name` (any session on the server — what tmux 3.x
    /// actually emits: `%%session-renamed $%u %s`); legacy form is the bare new
    /// name of the client's own session. Names may contain spaces, so the id is
    /// split off with maxSplits, never a blanket split.
    ///
    /// The prefix is stripped by name on purpose: this line used to
    /// `dropFirst(18)` against a 17-character prefix, which ate the first
    /// character of the name — and on a modern tmux left the session id in it.
    /// The Go port (daemon/internal/tmuxcm) has always done it this way.
    private func parseSessionRenamed(_ line: String) {
        let rest = String(line.dropFirst("%session-renamed ".count))
        let parts = rest.split(separator: " ", maxSplits: 1)
        if parts.count == 2, let sesID = TmuxSessionID(string: String(parts[0])) {
            onNotification?(.sessionRenamed(session: sesID, name: String(parts[1])))
            return
        }
        onNotification?(.sessionRenamed(session: nil, name: rest))
    }
