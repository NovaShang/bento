...
public enum TmuxNotification: Sendable {
    ...
    /// `session` is nil only for the legacy bare-name form (pre-3.x tmux); a
    /// modern server names the session that was renamed, which is not
    /// necessarily the one this client is attached to.
