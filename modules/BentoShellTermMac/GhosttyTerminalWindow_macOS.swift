// Ported from frozen GhosttyTerminalWindow_macOS.swift @ 8fa54b6^; data layer swapped, UI verbatim.
...
final class SessionTab {
    ...
    /// (The trunk surface carries onOpenPath/resolvePathClick closures instead
    /// of the context itself, so the plain tab's context lives here.)
    private var plainContext: PathPreviewContext?
    var previewContext: PathPreviewContext? {
        paneHost?.activePathPreviewContext ?? plainContext
    }
    ...
    init(choice: TmuxStartChoice, title: String, key: String? = nil, command: [String]? = nil) {
        ...
        // Data layer: the frozen LocalPtyTransport + tmux -CC stack is the
        // trunk daemon now — tmux tabs ride the structure mirror + pane byte
        // pipes, plain tabs a daemon-hosted pty (`spawn` kind=pty).
        let vm = TerminalViewModel(command: command, environment: env)
        self.viewModel = vm
        if choice == .noTmux {
            ...
            self.plainContext = context
            surface.onOpenPath = { path, line in
                BentoTerminalWindow.openPreview(path: path, line: line, context: context)
            }
            surface.resolvePathClick = { paths, rootHints in
                guard let res = try? await SmartPathResolver.resolveFirst(
                    paths: paths, rootHints: rootHints, context: context) else { return nil }
                return (res.index, res.resolvedPath)
            }
...
final class TerminalWindowManager: NSObject, NSWindowDelegate {
    ...
    /// Data-layer seam (flagged): one daemon control client per target means
    /// one streaming session at a time; the key window's session is the one
    /// the user is looking at, so it takes the attach (the frozen product's
    /// per-tab -CC clients needed nothing here).
    func windowDidBecomeKey(_ notification: Notification) {
        activeTab?.viewModel.ensureAttachedIfNeeded()
    }
