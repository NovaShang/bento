// Ported from frozen GhosttyTiledPaneHost_macOS.swift @ 8fa54b6^; data layer swapped, UI verbatim.
...
public final class GhosttyTiledPaneHost: NSView, NSMenuDelegate {
    public let viewModel: TerminalViewModel
    ...
    private func wireSurfaceCallbacks(_ surface: GhosttyTerminalSurface,
        ...
        // (The trunk surface carries onOpenPath/resolvePathClick closures
        // instead of the context itself; the context lives in `paneContexts`.)
    ...
    /// Per-pane file contexts (see wireSurfaceCallbacks) — torn down with the
    /// pane's cell (data-layer seam: the trunk surface holds closures, not the
    /// context).
    private var paneContexts: [TmuxPaneID: PathPreviewContext] = [:]
    ...
    func presentCommandPalette(from anchor: NSView? = nil) {
        let ctx = activePaneID.flatMap { paneContexts[$0] }
    ...
    var activePathPreviewContext: PathPreviewContext? {
        activePaneID.flatMap { paneContexts[$0] }
    }
