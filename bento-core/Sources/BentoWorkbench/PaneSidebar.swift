...
public struct PaneSidebar: View {
    ...
    /// uses for "Split — Path & Command" — injected by the Mac shell at
    /// startup (the workbench never imports shell chrome), routed to open a
    /// Focus-mode pane (or resume a past conversation in the chosen folder)
    /// instead of splitting — so both modes share one dialog and there's no
    /// bespoke SwiftUI form.
    public static var directoryPanelPresenter: ((
        _ title: String, _ prompt: String, _ initialDirectory: String?,
        _ onCreate: @escaping (String?, String?) -> Void,
        _ onResume: ((CatalogEntry) -> Void)?
    ) -> Void)?
    ...
    private func presentPathCommandPanel() {
        ...
        Task { @MainActor in
            ...
            Self.directoryPanelPresenter?(
                "New Agent", "Create", cwd,
                { path, command in
                    Task { await viewModel.newFocusPane(.custom(path: path, command: command)) }
                },
                { entry in
