...
public final class AgentChatSurface: NSView {
    ...
    /// Shell-injected route from a path click to the shared preview dock —
    /// the pane module never imports window chrome. Installed at startup by
    /// the Mac shell alongside the rest of the app wiring.
    public static var previewOpener: ((String, Int?, PathPreviewContext) -> Void)?
    ...
    private func openFilePreview(path: String, line: Int?) {
        ...
        Self.previewOpener?(path, line, context)
    }
}
